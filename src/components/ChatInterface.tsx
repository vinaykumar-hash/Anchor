...
export function ChatInterface() {
  ...
  const { messages, addMessage, selectedCapture, selectCapture, toggleHistory, isHistoryOpen, useGpu, toggleGpu } = useAppStore();
      ...
      if (userMsg.toLowerCase().startsWith('/search ')) {
        ...
        try {
          ...
          if (resultJson.results && resultJson.results.length > 0) {
            // Add assistant message with results formatted
            // We pass the results as a JSON string in the content so MessageBubble can parse it,
            // or we just format it as markdown text. Since we only have a string content, we can use a special prefix.
            addMessage({
        ...
        if (selectedCapture) {
          // This calls our rust backend which in turn calls the python script
          ...
          try {
            ...
            if (resultJson.answer) {
              addMessage({
                id: (Date.now() + 1).toString(),
                role: 'assistant',
                content: `RAG_RESULT:${JSON.stringify({answer: resultJson.answer, sources: resultJson.sources})}`,
                timestamp: Date.now()
              });
            } else {
              addMessage({
                id: (Date.now() + 1).toString(),
                role: 'assistant',
                content: `I couldn't process your question properly.`,
                timestamp: Date.now()
              });
            }
          } catch (e) {
            addMessage({
              id: (Date.now() + 1).toString(),
              role: 'assistant',
              content: `Failed to search your memory: ${e}`,
              timestamp: Date.now()
            });
          }
        }
